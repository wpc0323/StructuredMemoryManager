...
| `search_memory()` | 加权二级检索记忆 | Agent 需要回忆信息时 |
...
- **system.md 全面重写**：与代码实际实现对齐（独立文件存储 + entries 索引 + 二级检索），修正了 v1.0 单文件聚合架构的文档滞后问题
- **跨项目跨会话共享记忆**：移除按项目路径哈希分目录的逻辑，改为全局共享目录（Trae: `~/.trae-cn/memory/StructuredMemoryManager/`，Cursor: `~/.cursor/memory/StructuredMemoryManager/`，默认: `~/.agent-memory/StructuredMemoryManager/`），所有项目、所有对话使用同一份记忆文件
- **SKILL.md 修正**：修复 description（三级→二级检索）；补充 `read_memory` 工具方法定义；修复 `rebuild_index` 参数（`file_path`→`memory_dir`）；修复 `confirm_memory` 参数说明（`new_priority`、`mention_count`）；说明空查询行为（`query=""` 返回全部记忆）

**Bug 修复与健壮性改进：**

- **YAML 回退解析器**：增加整数和浮点数类型推断，修复 `mention_count: 3` 被解析为字符串的 bug
- **归档后索引同步**：归档文件移入 `archive/` 后同步更新总目录中对应条目的 path
- **归档第二轮降阈值**：首轮 90 天筛选后若仍超阈值，自动降至 60 天再筛选
- **confirm_memory 参数修正**：移除未使用的 `emphasis` 参数；`upgrade`/`downgrade` 操作现在使用 `new_priority` 参数（默认值分别为 high/low）
- **generate_entry_id 并发安全**：增加随机起始序号 + 冲突检测循环，降低并发 ID 冲突概率
- **移除测试文件**：删除 `test_weighted_search.py`
