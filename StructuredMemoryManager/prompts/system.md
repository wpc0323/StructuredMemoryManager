...
1. **调用 `add_memory(category, content, priority, tags, expires, related, emphasis, mention_count)`**
2. 工具内部自动完成以下全部步骤：
...
→ 读取 emphasis/mention_count，重新精确计算权重
→ 按权重排序（高权重优先）
→ 输出匹配的 entry_id 列表
```
...
→ 精确读取该条目完整内容
→ 返回 {file_path, entry_id, anchor, content, weight} 给 Agent
```
