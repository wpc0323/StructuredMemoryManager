...
try:
    from ._base import (
        ...
        ARCHIVE_THRESHOLD, CATEGORY_DIR_MAP
    )
except ImportError:
    from _base import (
        get_entry_file_path, read_memory_file, write_memory_file,
...
def _check_archiving(category: str) -> dict:
    ...
    将低优先级且过期的条目文件移入 archive 子目录
    """
    from _base import MEMORY_DIR, days_since, ARCHIVE_AGE_DAYS, read_memory_file

    cat_dir = MEMORY_DIR / CATEGORY_DIR_MAP.get(category, category)
...
def add_memory(
    ...
    # 2. 生成文件路径
    file_path = get_entry_file_path(category, entry_id, summary, project_name)
    ...
    _update_index_for_entry(file_rel_path, category, summary, priority, tags, entry_id, project_name)
