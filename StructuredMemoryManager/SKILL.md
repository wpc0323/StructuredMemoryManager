...
version: 1.0.0
description: "基于独立文件存储、YAML索引和加权二级检索机制的长期记忆管理skill，替代默认按时间分文件的记忆方式。当需要记录用户偏好、保存技能方法、检索历史记忆、维护记忆状态时调用此Skill。"
...
**返回值：** `list[dict]`，每项包含 `file_path, entry_id, content_snippet, score, weight, priority, tags, emphasis, mention_count`
...
| `new_priority` | string | 否 | null | 新优先级（upgrade时默认"high"，downgrade时默认"low"） |
| `mention_count` | integer | 否 | null | 提及次数（bump_mention时使用，默认当前值+1） |

---

### 4. read_memory — 读取单条记忆

读取单条记忆的完整内容。

**何时调用：**
- 需要查看某条记忆的完整详情（摘要不足以理解时）
- 需要确认记忆的具体内容后再做操作
- 检索结果中 content_snippet 被截断，需要读取完整正文
...
**返回值：** `dict`，包含 `entry_id, category, priority, tags, summary, content, emphasis, mention_count, expires, date, last_modified`

---

### 5. rebuild_index — 重建索引

扫描所有记忆文件，全量重建总目录 `memory_index.md` 的索引。
...
无必填参数。自动扫描所有类别目录（habits/、skills/、projects/）并重建索引。
